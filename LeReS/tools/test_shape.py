...
if __name__ == '__main__':
    ...
    imgs_list.sort()
    imgs_path = [os.path.join(image_dir, i)
                 for i in imgs_list if i != 'outputs']
    ...
    for i, v in enumerate(imgs_path):
        ...
        # save depth
        plt.imsave(os.path.join(image_dir_out,
                                img_name[:-4]+'-depth.png'), pred_depth_ori, cmap='rainbow')
        ...
        reconstruct_depth(
            depth_scaleinv, rgb[:, :, ::-1], image_dir_out, img_name[:-4]+'-pcd', focal=focal_length)
